...
public class GetByIdProductDetailQuery : IRequest<GetByIdProductDetailViewModel>
{
		...
		GetByIdProductDetailQueryHandler : IRequestHandler<GetByIdProductDetailQuery, GetByIdProductDetailViewModel>
	{
		...
		{
			var product = await _productRepository.Query().Where(p => p.Id == request.ProductId).Include(r => r.Reviews)!.ThenInclude(ru => ru.AppUser).Include(u => u.User).Include(c => c.Colors).Include(s => s.Sizes).Include(pi => pi.ProductImages).FirstOrDefaultAsync(cancellationToken: cancellationToken);
